...
# from ops.dcn.modules.modulated_deform_conv import ModulatedDeformConv
from model_edvr import EDVR
import numpy as np
# ==========
...
class STDF(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, base_ks=3, deform_ks=3):
        ...
        self.deform_conv = ModulatedDeformConv(
            in_nc, out_nc, deform_ks, 1, padding=deform_ks//2, deformable_groups=in_nc
            )
...
class MFVQE(nn.Module):
    ...
    def __init__(self, opts_dict):
        ...
        self.edvr = EDVR()
        #self.residual_fusion_1 = nn.Conv2d(6, 3, 3, 1, 1)
        #self.residual_fusion_2 = nn.Conv2d(3, 3, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        edvr_residual = self.edvr(x)
        out = self.ffnet(x_e)
        out = self.qenet(out)
        out += x_e[:, self.radius: self.radius + 1, ...]
        out += edvr_residual
        out = out/2
        return out


class MFVQE_2(nn.Module):
    """STDF -> QE -> residual.
    ...
    def forward(self, x, y):
        x_e = torch.unsqueeze(x, 2)
        enhanced_lr = self.edvr(x_e)
        y[:, self.radius: self.radius+1, ...] = enhanced_lr
        out = self.ffnet(x)
        out = self.qenet(out)
        out += enhanced_lr  # res: add middle frame
        return out
