...
foder_list = os.listdir(path0+'test_png1')
...
for a in b:
    ...
    path1 = path0 + 'test_png1/'+a
    fra_num = len(os.listdir(path1))
    list_psnr = []
    txt = path0+'test_hdf51/'+a+'_psnr.txt'
    ...
    save_path = path0+'test_hdf51/'+ a +'_label.hdf5'
    f = h5py.File(save_path, "w")
    f.create_dataset('PQF_label', data=PQF_label)
    f.close()
    '''
hdfFile = h5py.File('D:/NITER/validation_fixed-rate/test_hdf5/006_label.hdf5', 'r')
